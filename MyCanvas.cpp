...
//            Fill in pixels left to right
            if(paint.getShader()) {
                ...
                drawPixels(paint, storage, GRoundToInt(leftX), y, GRoundToInt(rightX) - GRoundToInt(leftX));
            } else {
                for(int x = GRoundToInt(leftX); x < GRoundToInt(rightX); x++) {
...
//                        cout << "- " << p.fA << " " << p.fR << " " << p.fG << " " << p.fB << endl;
                        ...
                        GPixel temp = GPixel_PackARGB(p.fA*255, p.fR*p.fA*255, p.fG*p.fA*255, p.fB*p.fA*255);
                        drawPixels(paint, &temp, x, y, 1);
                    }
    ...
    void drawPixels(GPaint paint, GPixel *pixel, int x, int y, int count) {
        // if(!layers.top().bounds->contains({x,y}) || layers.top().bounds->isEmpty())  {
        //     // cout << "fuck" << endl;
        //     return;
        // }
